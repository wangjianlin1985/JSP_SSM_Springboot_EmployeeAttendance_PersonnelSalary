...
public class ManagerService {
    ...
    /*按照查询条件分页查询部门经理记录*/
    public ArrayList<Manager> queryManager(String managerUserName,Department departmentObj,String name,String bornDate,String telephone,int currentPage) throws Exception { 
    	...
    	if(!bornDate.equals("")) where = where + " and t_manager.bornDate like '%" + bornDate + "%'";
    ...
    /*按照查询条件查询所有记录*/
    public ArrayList<Manager> queryManager(String managerUserName,Department departmentObj,String name,String bornDate,String telephone) throws Exception  { 
     	String where = "where 1=1";
    	if(!managerUserName.equals("")) where = where + " and t_manager.managerUserName like '%" + managerUserName + "%'";
    	if(null != departmentObj &&  departmentObj.getDepartmentNo() != null && !departmentObj.getDepartmentNo().equals(""))  where += " and t_manager.departmentObj='" + departmentObj.getDepartmentNo() + "'";
    	if(!name.equals("")) where = where + " and t_manager.name like '%" + name + "%'";
    ...
    /*当前查询条件下计算总的页数和记录数*/
    public void queryTotalPageAndRecordNumber(String managerUserName,Department departmentObj,String name,String bornDate,String telephone) throws Exception {
     	String where = "where 1=1";
    	if(!managerUserName.equals("")) where = where + " and t_manager.managerUserName like '%" + managerUserName + "%'";
    	if(null != departmentObj &&  departmentObj.getDepartmentNo() != null && !departmentObj.getDepartmentNo().equals(""))  where += " and t_manager.departmentObj='" + departmentObj.getDepartmentNo() + "'";
    	if(!name.equals("")) where = where + " and t_manager.name like '%" + name + "%'";
	...
	public boolean checkLogin(String username, String password) throws Exception { 
		Manager db_manager = (Manager) managerMapper.getManager(username);
		if(db_manager == null) { 
			this.errMessage = " 账号不存在 ";
			System.out.print(this.errMessage);
			return false;
		} else if( !db_manager.getPassword().equals(password)) {
