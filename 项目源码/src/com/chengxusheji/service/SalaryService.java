...
public class SalaryService {
    ...
    /*按照查询条件分页查询工资记录*/
    public ArrayList<Salary> querySalary(Employee employeeObj,String salaryYear,String salaryMonth,int currentPage) throws Exception { 
    	...
    	if(!salaryYear.equals("")) where = where + " and t_salary.salaryYear like '%" + salaryYear + "%'";
    	if(!salaryMonth.equals("")) where = where + " and t_salary.salaryMonth like '%" + salaryMonth + "%'";
    	int startIndex = (currentPage-1) * this.rows;
    	return salaryMapper.querySalary(where, startIndex, this.rows);
    }

    /*按照查询条件查询所有记录*/
    public ArrayList<Salary> querySalary(Employee employeeObj,String salaryYear,String salaryMonth) throws Exception  { 
     	String where = "where 1=1";
    	if(null != employeeObj &&  employeeObj.getEmployeeNo() != null && !employeeObj.getEmployeeNo().equals(""))  where += " and t_salary.employeeObj='" + employeeObj.getEmployeeNo() + "'";
    	if(!salaryYear.equals("")) where = where + " and t_salary.salaryYear like '%" + salaryYear + "%'";
    	if(!salaryMonth.equals("")) where = where + " and t_salary.salaryMonth like '%" + salaryMonth + "%'";
    ...
    /*当前查询条件下计算总的页数和记录数*/
    public void queryTotalPageAndRecordNumber(Employee employeeObj,String salaryYear,String salaryMonth) throws Exception {
     	String where = "where 1=1";
    	if(null != employeeObj &&  employeeObj.getEmployeeNo() != null && !employeeObj.getEmployeeNo().equals(""))  where += " and t_salary.employeeObj='" + employeeObj.getEmployeeNo() + "'";
    	if(!salaryYear.equals("")) where = where + " and t_salary.salaryYear like '%" + salaryYear + "%'";
    	if(!salaryMonth.equals("")) where = where + " and t_salary.salaryMonth like '%" + salaryMonth + "%'";
