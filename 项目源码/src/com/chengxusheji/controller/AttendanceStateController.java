...
public class AttendanceStateController extends BaseController {
	...
	public void listAll(HttpServletResponse response) throws Exception {
		...
		for(AttendanceState attendanceState:attendanceStateList) {
			...
			jsonAttendanceState.accumulate("attendanceStateId", attendanceState.getAttendanceStateId());
			jsonAttendanceState.accumulate("attendanceStateName", attendanceState.getAttendanceStateName());
			jsonArray.put(jsonAttendanceState);
		}
	...
	@RequestMapping(value="/{attendanceStateId}/frontshow",method=RequestMethod.GET)
	public String frontshow(@PathVariable Integer attendanceStateId,Model model,HttpServletRequest request) throws Exception {
		/*根据主键attendanceStateId获取AttendanceState对象*/
        AttendanceState attendanceState = attendanceStateService.getAttendanceState(attendanceStateId);

        request.setAttribute("attendanceState",  attendanceState);
        return "AttendanceState/attendanceState_frontshow";
	}

	/*ajax方式显示出勤状态修改jsp视图页*/
	@RequestMapping(value="/{attendanceStateId}/update",method=RequestMethod.GET)
	public void update(@PathVariable Integer attendanceStateId,Model model,HttpServletRequest request,HttpServletResponse response) throws Exception {
        /*根据主键attendanceStateId获取AttendanceState对象*/
        AttendanceState attendanceState = attendanceStateService.getAttendanceState(attendanceStateId);
	...
	/*ajax方式更新出勤状态信息*/
	@RequestMapping(value = "/{attendanceStateId}/update", method = RequestMethod.POST)
    ...
    /*删除出勤状态信息*/
	@RequestMapping(value="/{attendanceStateId}/delete",method=RequestMethod.GET)
	public String delete(@PathVariable Integer attendanceStateId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  attendanceStateService.deleteAttendanceState(attendanceStateId);
	            request.setAttribute("message", "出勤状态删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "出勤状态删除失败!");
				return "error";
	...
	public void delete(String attendanceStateIds,HttpServletRequest request,HttpServletResponse response) throws IOException, JSONException {
		String message = "";
    	boolean success = false;
        try { 
        	int count = attendanceStateService.deleteAttendanceStates(attendanceStateIds);
        	success = true;
	...
	public void OutToExcel( Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        String[] headers = { "出勤状态id","出勤状态名称"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<attendanceStateList.size();i++) {
        	AttendanceState attendanceState = attendanceStateList.get(i); 
        	dataset.add(new String[]{attendanceState.getAttendanceStateId() + "",attendanceState.getAttendanceStateName()});
        }
        /*
