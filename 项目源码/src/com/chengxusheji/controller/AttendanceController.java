...
public class AttendanceController extends BaseController {
	...
	public void list(String attendanceDate,@ModelAttribute("employeeObj") Employee employeeObj,@ModelAttribute("attendanceState") AttendanceState attendanceState,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (attendanceDate == null) attendanceDate = "";
		if(rows != 0)attendanceService.setRows(rows);
		List<Attendance> attendanceList = attendanceService.queryAttendance(attendanceDate, employeeObj, attendanceState, page);
	    /*计算总的页数和总的记录数*/
	    attendanceService.queryTotalPageAndRecordNumber(attendanceDate, employeeObj, attendanceState);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String attendanceDate,@ModelAttribute("employeeObj") Employee employeeObj,@ModelAttribute("attendanceState") AttendanceState attendanceState,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (attendanceDate == null) attendanceDate = "";
		List<Attendance> attendanceList = attendanceService.queryAttendance(attendanceDate, employeeObj, attendanceState, currentPage);
	    /*计算总的页数和总的记录数*/
	    attendanceService.queryTotalPageAndRecordNumber(attendanceDate, employeeObj, attendanceState);
	    /*获取到总的页码数目*/
	    int totalPage = attendanceService.getTotalPage();
	...
	/*前台员工按照查询条件分页查询员工出勤信息*/
	@RequestMapping(value = { "/empFrontlist" }, method = {RequestMethod.GET,RequestMethod.POST})
	public String empFrontlist(String attendanceDate,@ModelAttribute("employeeObj") Employee employeeObj,@ModelAttribute("attendanceState") AttendanceState attendanceState,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (attendanceDate == null) attendanceDate = "";
	    ...
	    List<Employee> employeeList = employeeService.queryAllEmployee();
	    request.setAttribute("employeeList", employeeList);
		return "Attendance/attendance_empFrontquery_result"; 
	}
	...
	public void OutToExcel(String attendanceDate,@ModelAttribute("employeeObj") Employee employeeObj,@ModelAttribute("attendanceState") AttendanceState attendanceState, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(attendanceDate == null) attendanceDate = "";
        List<Attendance> attendanceList = attendanceService.queryAttendance(attendanceDate,employeeObj,attendanceState);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Attendance信息记录"; 
        String[] headers = { "出勤日期","考勤员工","出勤结果"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<attendanceList.size();i++) {
        	Attendance attendance = attendanceList.get(i); 
        	dataset.add(new String[]{attendance.getAttendanceDate(),attendance.getEmployeeObj().getName(),attendance.getAttendanceState().getAttendanceStateName()});
        }
        /*
