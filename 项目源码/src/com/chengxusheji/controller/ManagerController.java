...
public class ManagerController extends BaseController {
	...
	/*ajax方式按照查询条件分页查询部门经理信息*/
	@RequestMapping(value = { "/list" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void list(String managerUserName,@ModelAttribute("departmentObj") Department departmentObj,String name,String bornDate,String telephone,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<Manager> managerList = managerService.queryManager(managerUserName, departmentObj, name, bornDate, telephone, page);
	    /*计算总的页数和总的记录数*/
	    managerService.queryTotalPageAndRecordNumber(managerUserName, departmentObj, name, bornDate, telephone);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String managerUserName,@ModelAttribute("departmentObj") Department departmentObj,String name,String bornDate,String telephone,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (managerUserName == null) managerUserName = "";
		if (name == null) name = "";
		if (bornDate == null) bornDate = "";
		if (telephone == null) telephone = "";
		List<Manager> managerList = managerService.queryManager(managerUserName, departmentObj, name, bornDate, telephone, currentPage);
	    /*计算总的页数和总的记录数*/
	    managerService.queryTotalPageAndRecordNumber(managerUserName, departmentObj, name, bornDate, telephone);
	    /*获取到总的页码数目*/
	    int totalPage = managerService.getTotalPage();
	...
	public String delete(@PathVariable String managerUserName,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  managerService.deleteManager(managerUserName);
	            request.setAttribute("message", "部门经理删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "部门经理删除失败!");
				return "error";
	...
	/*按照查询条件导出部门经理信息到Excel*/
	@RequestMapping(value = { "/OutToExcel" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void OutToExcel(String managerUserName,@ModelAttribute("departmentObj") Department departmentObj,String name,String bornDate,String telephone, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(managerUserName == null) managerUserName = "";
        if(name == null) name = "";
        if(bornDate == null) bornDate = "";
        if(telephone == null) telephone = "";
        List<Manager> managerList = managerService.queryManager(managerUserName,departmentObj,name,bornDate,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Manager信息记录"; 
        String[] headers = { "经理用户名","所属部门","姓名","性别","出生日期","联系电话"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<managerList.size();i++) {
        	Manager manager = managerList.get(i); 
        	dataset.add(new String[]{manager.getManagerUserName(),manager.getDepartmentObj().getDepartmentName(),manager.getName(),manager.getSex(),manager.getBornDate(),manager.getTelephone()});
        }
        /*
